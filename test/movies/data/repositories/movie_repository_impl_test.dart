...
import 'package:remote_content_explorer/movies/data/models/movie_model.dart';
import 'package:remote_content_explorer/movies/data/models/movie_response.dart';
import 'package:remote_content_explorer/movies/data/repositories/movie_repository_impl.dart';
...
void main() {
  ...
  group('MovieRepositoryImpl', () {
    test(
      'given the datasource returns a movie response '
      'when getNowPlaying is called '
      'then returns Right with the mapped movies',
      () async {
        // given
        when(() => mockDatasource.getNowPlaying(page: any(named: 'page')))
    ...
    test(
      'given the datasource throws a DioException '
      'when getNowPlaying is called '
      'then returns Left with the appropriate failure',
      () async {
        // given
        when(() => mockDatasource.getNowPlaying(page: any(named: 'page')))
            .thenAnswer(
    ...
    test(
      ...
      () async {
        // given
        when(() => mockDatasource.getPopular(page: any(named: 'page')))
            .thenAnswer((_) async => _tMovieResponse());
    ...
    test(
      ...
      () async {
        // given
        when(() => mockDatasource.searchMovies(query: any(named: 'query')))
            .thenAnswer((_) async => _tMovieResponse());
    ...
    test(
      'given the datasource returns a cast response '
      'when getMovieCast is called '
      'then returns Right with the mapped actors',
      () async {
        // given
        when(() => mockDatasource.getMovieCast(any()))
